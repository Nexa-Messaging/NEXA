...
export default function GroupChatScreen() {
  ...
  const [reportItem, setReportItem] = useState<GroupMessageFeed | null>(null);
  ...
  useEffect(() => {
    ...
    let active = true;
    void isConversationMuted('group', chatId).then(({ muted: mutedState, error }) => {
      if (active && !error) {
        setMuted(mutedState);
      }
    });
  ...
  const toggleMute = () => {
    ...
    setMuteBusy(true);
    void setConversationMuted('group', chatId, !muted).then((error) => {
      setMuteBusy(false);
      if (error) {
  ...
  const submitReport = (category: ReportCategory, details?: string) => {
    ...
    void reportGroupMessage(reportItem.id, category, details).then((error) => {
  ...
  // Header info + members list (drives sender names, roles and permissions).
  useEffect(() => {
    if (!chatId) {
      return;
    }
    ...
    void Promise.all([fetchGroupChatInfo(chatId), fetchGroupMembers(chatId)]).then(
      ([infoResult, memberResult]) => {
        if (!active) {
          return;
        }
        setGroupLoading(false);
        setGroup(infoResult.error ? null : (infoResult.data ?? null));
  ...
  // Resolve the group photo (signed URL) whenever it changes.
  useEffect(() => {
  ...
  useEffect(() => {
    for (const message of chat.messages) {
      ...
      void resolveGroupMediaUrl(message.id, message.media_path).then((result) => {
        if (result.url) {
  ...
  const toggleReaction = async (
    message: GroupMessageFeed,
    emoji: string,
    hasMine: boolean,
  ) => {
    setActionError(null);
    const error = hasMine
      ? await unreactToGroupMessage(message.id, emoji)
      : await reactToGroupMessage(message.id, emoji);
    if (error) {
      setActionError(error);
    }
  };

  const onDelete = async (message: GroupMessageFeed) => {
    Alert.alert('Delete message?', 'This removes the message for everyone in this group.', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: () => {
          setSheetItem(null);
          setActionError(null);
          void deleteGroupMessage(message.id).then((error) => {
            if (error) {
              setActionError(error);
            }
          });
  ...
  const renderItem = ({ item }: { item: ChatItem }) => {
    ...
    const real = 'status' in item ? null : (item as GroupMessageFeed);
      ...
      <MessageBubble
        ...
        onLongPress={() => {
          if (pendingFailed) {
            ...
            setSheetItem(item as GroupMessageFeed);
          }
        }}
        onRetry={
          pendingFailed && 'status' in item
            ? () => chat.retry(item as PendingGroupMessage)
            : undefined
        }
